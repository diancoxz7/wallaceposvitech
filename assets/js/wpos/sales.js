...
function WPOSItems() {
    ...
    function addItemRow(qty, name, unit, taxid, sitemid, data) {
        ...
        var row = $('<tr class="item_row">' +
            '<td><input class="itemid" type="hidden" value="' + sitemid + '" data-options=\''+JSON.stringify(data)+'\' /><input onChange="WPOS.sales.updateSalesTotal();" style="width:80px;" type="text" class="itemqty numpad" value="' + qty + '" /></td>' +
            ...
            '<td><input '+((disableprice==true && unit!="")?"disabled":"")+' onChange="WPOS.sales.updateSalesTotal();" style="max-width:80px;" type="text" class="itemunit numpad" value="' + unit + '" /></td>' +
            ...
            '<td><input style="max-width:80px;" type="text" class="itemprice" value="0.00" disabled /></td>' +
...
function WPOSSales() {
    ...
    function addPaymentRow(method, value, tender, change, extraData){
        ...
        var payrow =  '<tr '+data+'><td>' +
            ...
            '<div style="width: 100px; display: inline-block;">Tendered:</div><input onChange="WPOS.sales.updatePaymentChange($(this).parent());" class="paytender numpad" style="width:80px;" type="text" value="'+(method!='cash'?0.00:(tender!=null?tender:value))+'" />' +
            '<div style="width: 100px; display: inline-block;">Change:</div><input class="paychange" style="width:80px;" type="text" value="'+(method!='cash'?0.00:(change!=null?change:0.00))+'" readonly />' +
            '</div></td>' +
            '<td>'+curBefore+'<input onChange="WPOS.sales.updatePaymentSums();" class="payamount numpad" style="width:80px;" type="text" value="'+value+'" autocomplete="off"/>'+curAfter+'</td>' +
